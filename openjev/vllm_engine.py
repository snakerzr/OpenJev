"""vLLM gateway backend: batched ``/v1/completions`` + logprob slicing."""
...
class VLLMDecisionEngine:
    ...
    def __enter__(self) -> VLLMDecisionEngine:
        return self
    ...
    def health(self) -> dict[str, Any]:
        ...
        ready = any(mid == self.model_name for mid in ids) if ids else bool(models)
    ...
    def _decode_choice(self, item: _PreparedItem, choice: Any, temperature: float) -> Answer:
        ...
        # OpenAI API: string -> logprob (may be str keys with leading space)
